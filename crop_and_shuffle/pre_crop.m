...
source_prefix2 = '/media/storage/zhiweid/CollectiveActivityDataset/annote/Annotation2/';
...
video_train2 = [45:51 54:58 61:64 67 69];

video_val2 = setdiff([45:72],video_train2);

videos = [video_train video_val  video_train2 video_val2];
...
for k = videos
    ...
    try
        if k <= 44 
            ...
            load([source_prefix2 'annot_video' num2str(k) '.mat']);
        end
    catch
...
%             end
            ...
            if k > 44 || data_label(i).action(j) == 6
                label = label-1;
            end
            
            assert(label>=0);
            imwrite(patch,sprintf([patches_path 'patches/%d.jpg'], num));
            if data_label(i).action(j) == 5
                continue;
            end
    ...
    save(['annotation_distance/annot_video' num2str(k) '.mat'],'data_label');
end
